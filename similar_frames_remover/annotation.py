...
def create_filename_parsers() -> (
    List[Tuple[re.Pattern, Callable[[str], Tuple[str, int]]]]
):
    ...
    parsers = [
        ...
        (
            re.compile("^c[0-9]*_[0-9]{4}(_[0-9]{2}){2}_(_[0-9]{2}){3}$"),
            parse_hr_format,
        ),
    ]
...
def parse_filename(
    filename: str, parsers: List[Tuple[re.Pattern, Callable[[str], Tuple[str, int]]]]
...
def create_annotation(
    img_directory: str, min_img_side_size=300
    ...
    for fname in os.listdir(img_directory):
        ...
        if np.any(np.asarray(img_size_wh) < min_img_side_size):
            ...
            logging.info(
                f"Image {fname} is skipped due to small size min({img_size_wh}) < {min_img_side_size}"
            )
            continue
        ...
        if cam_id is None or timestamp_ms is None:
            ...
            logging.info(
                f"Image {fname} is skipped due to incompatible file name format"
            )
            continue

        hr_timestamp = datetime.datetime.fromtimestamp(timestamp_ms / 1000).strftime(
            "%Y_%m_%d__%H_%M_%S"
        )
        sequences[cam_id].append(
            FrameAnnotation(
                image_name=fname,
                epoch_timestamp_ms=timestamp_ms,
                hr_timestamp=hr_timestamp,
                orig_img_size_wh=img_size_wh,
            )
        )

    for cam_id in sequences.keys():
        sequences[cam_id] = list(
            sorted(sequences[cam_id], key=lambda s: s.epoch_timestamp_ms)
        )

    logging.info(
        f'Annotation is created, num samples per sequences: {[f"{cam_id}: {len(sequences[cam_id])}" for cam_id in sorted(sequences.keys())]}'
    )
    return sequences
