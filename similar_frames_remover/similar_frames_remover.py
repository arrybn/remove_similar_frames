...
from similar_frames_remover.annotation import (FrameAnnotation,
                                               create_annotation)
from similar_frames_remover.img_processing import (
    create_caching_load_preprocess, get_min_area_img_size)
...
def compare_imgs(img1: np.ndarray, img2: np.ndarray, min_cnt_area_ratio: float):
    ...
    score_nonnormed, contours, thresholded_diff = compare_frames_change_detection(
        img1, img2, imgs_area * min_cnt_area_ratio
    )
...
def remove_subsequent_similar_frames(
    img_seq: List[FrameAnnotation],
    dataset_path: str,
    resize_img_size_wh: Tuple[int, int],
    load_imgs_function: Callable[
        [str, str, Tuple[int, int], Union[float, None]], np.ndarray
    ],
    ...
    for prev_sample, next_sample in zip(img_seq, img_seq[1:]):
        ...
        similar = are_imgs_similar(
            load_imgs_function(
                prev_sample.image_name,
                dataset_path,
                resize_img_size_wh,
                gaussian_blur_ksize_ratio,
            ),
            load_imgs_function(
                next_sample.image_name,
                dataset_path,
                resize_img_size_wh,
                gaussian_blur_ksize_ratio,
            ),
    ...
    logging.info(
        f"Removed {len(img_seq) - len(unique_sebsequent_frames)} frames from {len(img_seq)}"
    )

    return unique_sebsequent_frames


def remove_similar_frames_by_clustering(
    img_seq: List[FrameAnnotation],
    dataset_path: str,
    resize_img_size_wh: Tuple[int, int],
    load_imgs_function: Callable[
    ...
    def metric(index_1, index_2):
        ...
        img1 = load_imgs_function(
            sample_1.image_name,
            dataset_path,
            resize_img_size_wh,
            gaussian_blur_ksize_ratio,
        )
        img2 = load_imgs_function(
            sample_2.image_name,
            dataset_path,
            resize_img_size_wh,
            gaussian_blur_ksize_ratio,
    ...
    logging.info(
        f"Removed {len(img_seq) - len(unique_images_annotation)} frames from {len(img_seq)}"
    )
...
class SimilarFramesRemover:
    def __init__(
        self,
        gaussian_blur_hksize_ratio: float,
        min_cnt_area_ratio: float,
        similar_score_thr: float,
    ...
    def process_directory(self, imgs_dir_path):
        ...
        for cam_id in sorted(full_annotation.keys()):
            ...
            subseq_unique_samples = remove_subsequent_similar_frames(
                full_annotation[cam_id],
                imgs_dir_path,
                img_size,
                self.load_prep_func,
                self.gaussian_blur_hksize_ratio,
                self.min_cnt_area_ratio,
                self.similar_score_thr,
            )

            cam_unique_frames = remove_similar_frames_by_clustering(
